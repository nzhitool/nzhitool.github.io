...
**N指助手**是一款免费的国内顶尖键鼠映射软件，可以为玩家带来强大的手柄映射功能，软件将为用户提供经典的简易键盘映射系统；它们的设计有相应的控制图标和全方位的功能。您所要做的就是移动这些键盘以适应手机屏幕上的控制器。

> **软件特色**
> 
> 软件运行于Windows平台，支持键盘鼠标玩安卓设备吃鸡手游
> 
> 非模拟器，可区别为配普通手机、平板玩家
> 
> 支持键位自定义,上下车不卡视角，舔包不卡视角
> 
> 压枪及连点功能支持记忆枪位和开镜状态，开镜压枪，不开镜不压枪，丢雷不压枪
> 
> 鼠标呼出功能支持开背包地图自动呼出鼠标
> 
> 免费版与收费版均包含：按键压枪、识别压枪、连点射击、呼出鼠标、键盘宏组件等所有功能

目前市面上所有投屏的软件几乎无法媲美Nzhi，例如 TCgamesa：连接繁琐，经常无法操控（Nzhi有他所有VIP的功能）

免费版与收费版唯一区别：免费版只能数据线连接，收费版可以WIFI或者网线连接，用于设备投屏。免费版适用于不支持扩展坞的手机或者平板，支持扩展坞的设备则推荐收费版

> **使用说明**
> 
> 为什么选择映射而不是王座？
> 
> 1.王座要经常更新固件，不更新容易拉闸，映射永不封号，无视游戏更新
> 
> 2.王座不需要电脑，映射需要电脑
> 
> 3.王座不能息屏投屏，映射可以息屏投屏（更省电，更丝滑，发热比王座小，耗电也比王座慢）
> 
> 4.王座带识别压枪，但是压枪没有映射准
> 
> 5.映射也有识别压枪，但是需要自己添加数据后才可以实现，映射快捷键压枪精调后比王座准10倍


## 使用Nzhi必须的配置:

### 1.接口
自己的手机或者平板必须支持3.0TPC接口
<img src="https://img.picui.cn/free/2024/06/25/6679a409ed233.png" width = 526 height = 885>

### 2.拓展坞
扩展坞种类很多，自行挑选
扩展坞选择支持4K60HZ，3.0USB接口，带PD100W快充，带网口，也可以串联使用，单独使用USB网卡
<img src="https://img.picui.cn/free/2024/06/25/6679a596988a8.png" width = 322 height = 238>
...
```
问：手机连接拓展坞投屏短暂黑屏怎么解决？
答：1. 电缆连接不良：检查连接电缆是否插紧，是否有松动或损坏。
    2. 电源问题：检查拓展坞的电源是否正常，是否有电源故障。
    3. 显示器问题：检查显示器是否正常，是否有显示器故障。
    4. 驱动程序问题：部分拓展坞是需要安装驱动的，驱动程序是否正确安装，是否需要更新。
解决方法：
    1. 检查连接电缆是否插紧，是否有松动或损坏，重新插拔电缆。
    2. 检查拓展坞的电源是否正常，是否有电源故障，更换电源或修理电源故障。
    3. 检查显示器是否正常，是否有显示器故障，更换显示器或修理显示器故障
```

```
问：画质、流畅度、帧率问题
答：       画质：HDMI（DP）> USB 投屏 > WIFI 投屏
         流畅度：HDMI（DP）> WIFI 投屏 > USB 投屏
     帧率稳定性：HDMI（DP）> USB 投屏 > WIFI 投屏

HDMI（DP） 需要手机支持，需要额外的手机 HDMI（DP） 拓展坞，手机是否支持可以询问拓展坞的卖家，扩展坞必须选择支持4K60HZ的，最好带有网口和PD快充100W，HDMI2.1的线，DP2.1的线.

高帧需要配合的，也就是显示器、手机（平板）、扩展坞、线都要支持
```

<img src="https://img.picui.cn/free/2024/06/25/6679aeb007686.png" width = 842 height = 411>
...
答：
    1 , 找到“关于手机”
    进入华为手机的设置页面，往下面翻动，找到关于手机的选项并点击。

    2 , 点击版本号
    等页面跳转后，再在里面找到版本号，用手多点几次版本号的位置 , 下面就会跳出提示框，提示您已经处于开发者模式了。

    3 , 进入开发者模式
    返回设置 , 下拉找到系统与更新 , 点击进入 , 页面跳转 , 再次下拉找到开发人员选项

    4 , 打开USB调试
    打开开发人员选项 , 下拉找到USB调试 , 同时打开"仅充电"模式下允许ADB调试
```

```
问：小米系列手机(包括红米,黑鲨)如何打开USB调试
答：
    1,点击我的设备
    进入设置主页面，点击我的设备，点击全部参数。

    2,点击MIUI版本
    连续点击MIUI版本直到出现提示，开发者权限已开启。
    ...
    5,打开USB调试
    在设置中下拉找到USB调试,并打开,同时打开USB调试(安全设置)
```
